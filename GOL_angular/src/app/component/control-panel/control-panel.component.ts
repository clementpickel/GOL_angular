...
export class ControlPanelComponent {
  ...
  form: FormGroup = new FormGroup({
    ...
    size: new FormControl(this.gol.caseSize, [Validators.required, Validators.min(1)])
  })
  ...
  constructor(public gol: GolService) {
    ...
    this.form.get("size")?.statusChanges.subscribe( status => {
      if (status == "VALID") {
        gol.caseSize = Number(this.form.get("size")!.value)
      }
    })
  }
}
